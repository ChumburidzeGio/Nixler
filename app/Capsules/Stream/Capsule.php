...
class Capsule {
     ...
     * @return this
     */
    public function whereNotIds(array $ids)
    {
        $this->model = $this->model->whereNotIn('p.id', $ids);
    ...
    public function recommendedFor($id)
    {
    	...
    	})->orderBy('f.id', 'desc')->addSelect('f.id');
    ...
    private function filter()
    {
    	...
    	$query = $this->model->distinct('p.id');
     ...
     * @return this
     */
    public function get($transformed = true)
    {
        $this->filter();

        $this->items = $this->cache('items', 0, function() use ($transformed) {

            $paginate = $this->model->skip(($this->page - 1) * $this->perPage)->simplePaginate($this->perPage);

            return $transformed ? $this->transform($paginate->items()) : $paginate->items();
